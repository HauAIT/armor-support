...
import {hasArmorDependency} from './env';
 ...
 */
export const CACHE_DIR_RELATIVE_PATH = path.join('node_modules', '.cache', 'armor');
...
export class NPM {
  ...
  async exec (cmd, args, opts, execOpts = {}) {
    ...
    let runner = async () => await exec(npmCmd, args, aitProcessExecOpts);
    if (lockFile) {
    ...
    try {
      ...
      const {
        ...
        stderr = '',
        code = null,
      } = /** @type {import('ait-process').AITProcessExecError} */ (e);
      const err = new Error(
  ...
  async installPackage (cwd, pkgName, {pkgVer, installType} = {}) {
    ...
    if (!(await hasArmorDependency(cwd))) {
